...
static int initFAT() {
  ...
  // Memcard in slot A or B, SD2SP2 in Serial Port 2
  __io_gcsd2.startup();
  if (__io_gcsd2.isInserted()) {
    if (fatMountSimple("fat", &__io_gcsd2)) {
      // SD2SP2 mounted successfully
      return 1;
    }
  }
  ...
  if (slotagecko) { // Memcard in SLOT B, SD gecko in SLOT A
    // This will ensure SD gecko is recognized if inserted or changed to another
    // slot after GCMM is executed
    for (i = 0; i < 10; i++) {
